...
export default {
  ...
  computed: {
    returnHref() {
      return { name: 'courseworkView', params: this.$route.params, query: { ...this.$route.query, tab: 'milestones' } };
    },
  },
  methods: {
    /**
    ...
    async confirm() {
      ...
      const response = await this.$store.dispatch('post', { url: '/milestone/create', data: { coursework: this.coursework.id, ...data } });
      if (response.error) {
        ...
        this.coursework.milestones.push(response.result);
