<template>
  <div class="delete-participant-window-vue">
    <div class="header">
      <span class="action">
        You are about to uninvite
        <span class="lighten">{{ member.username }}</span>
      </span>
    </div>
    <div class="body">
      <div class="info">This action cannot be undone. The user will not have private access to this coursework anymore!</div>
    ...
    <div class="footer">
      <Button v-bind="buttons.confirm">I understand, uninvite the member</Button>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      buttons: {
        confirm: { name: 'confirm-uninvite', icon: 'trash', type: 'alert dialog', click: () => this.confirm() },
      },
      fields: {
        username: { id: 'cwmd-username', icon: 'person', type: 'text', placeholder: this.member.username },
      },
    };
  },
  methods: {
    ...
    async confirm() {
      ...
      const response = await this.$store.dispatch('post', { url: '/coursework/deleteParticipant', data: { coursework: this.coursework.id, member: this.member.id, username: this.fields.username.model } });
      if (response.error) {
        ...
        this.$set(this.coursework, 'participants', _.reject(this.coursework.participants, (cwm) => cwm.id === this.member.id));
